#!/usr/bin/env python3
"""Export comparison CSV tables from existing pipeline evaluation summaries."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Export run-level and task-level comparison CSV tables from pipeline evaluation summaries."
    )
    parser.add_argument(
        "--runs",
        default="",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to logs/pipeline/comparison/runs_<timestamp>.csv"
        ),
    )
    parser.add_argument(
        "--task-csv",
        ...
        help=(
            ...
            "Defaults to logs/pipeline/comparison/tasks_<timestamp>.csv"
        ),
    )
...
def main() -> int:
    ...
    if not run_dirs:
        raise FileNotFoundError(
            "No evaluation summaries found. Run stage 2 first: scripts/pipeline/run_ollama_evaluation_pipeline.py"
        )
